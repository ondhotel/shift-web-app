"""
reservation_parser.py
予約CSVを読み込んで日別集計に変換するモジュール
"""
import pandas as pd
import re
...
def extract_ci_co(row) -> tuple:
    """
    C/I, C/O を抽出する。
    1. C/I列が有効な日付ならそのまま使用
    2. #######の場合はOTA備考/メモから ZZ2026/04/XX パターンで復元
    3. それも無ければ None
    """
...
def load_reservation_csv(filepath_or_buffer) -> pd.DataFrame:
    """
    予約CSVまたはExcel（当月分CSVシート）を読み込んで正規化DataFrameを返す。
    期待するカラム: #予約番号, プラン名, C/I, C/O, 大人人数, 子供人数, 部屋タイプ, ステータス
    """
    # ファイル種別判定
...
def build_daily_summary(df: pd.DataFrame) -> pd.DataFrame:
    """
    日別に宿泊・人数・部屋数・肉割烹・ライト・日帰りを集計して返す。
    宿泊はC/I〜C/Oの全泊を展開（複数泊は各日にカウント）。
    """
    rows = []
    ...
    if detail_df.empty:
        return pd.DataFrame(columns=["日付","宿泊室数","宿泊人数","日帰り室数","日帰り人数",
