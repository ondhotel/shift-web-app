...
def render_calendar_component(df: pd.DataFrame, staff_list: list, dept_list: list, gas_url: str):
    ...
    # 3. HTML/JSの組み立て
    # 💡 Pythonの f-string ではなく、通常の文字列として定義することで波括弧のエラーを完全に防ぎます
...
body { background:var(--bg); color:var(--tx); font-family:var(--fn); font-size:12px; height:__HEIGHT__px; overflow:hidden; }
...
window.onload = render;
    ...
    # 4. 文字列置換でデータを注入（これで f-string のエラーを回避）
    final_html = html_content.replace("__HEIGHT__", str(H)).replace("__SHIFTS_DATA__", shifts_json_str)
