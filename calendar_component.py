...
.mc.psel,.wdcol.psel,.sw-day-cell.psel{{background:var(--sel)!important;outline:2px solid var(--cp);outline-offset:-2px;}}
...
.fl{{display:block;font-size:10px;font-weight:700;color:var(--tx2);text-transform:uppercase;margin-bottom:4px;}}
...
<div id="app">
  <div id="topbar">
    ...
    <div id="pbanner"><span class="pbl">📋 ペーストモード</span><span class="pbcnt" id="pcnt"></span><button class="pbb exec" id="pexec" style="display:none">実行</button><button class="pbb cancel" id="pcancel">✕</button></div>
...
function onCalClick(e) {{
  ...
  if(chip) {{
    ...
    if(!isNaN(idx)) {{ if(clip) togglePasteDate(fmt(pd_(SHIFTS[idx].start))); else showDet(SHIFTS[idx]); return; }}
  }}
...
function renderDay() {{
  ...
  if(ds===td){{ const n=new Date(), diff=(n.getHours()*60+n.getMinutes()); const nl=mk('div','nowl'); nl.style.top=(diff/60*HPX)+'px'; db.appendChild(nl); }}
}}
